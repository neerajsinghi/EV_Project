...
func NewRepository(collectionName string) PlanRepository {
	return &repo{
...
func (r *repo) UpdateOne(filter, update bson.M) (string, error) {
	...
	if result.MatchedCount == 1 && result.ModifiedCount == 0 {
		err = errors.New("nothing to modify(304)")
		trestCommon.ECLog3(
			"update profile",
			err,
			logrus.Fields{
		...
		return "nothing to modify(304)", nil
	}
	return "updated successfully", nil
}

// used by get profile ,login and email verification
func (r *repo) FindOne(filter, projection bson.M) (entity.PreDefNotification, error) {
	var profile entity.PreDefNotification
...
func (r *repo) Find(filter, projection bson.M) ([]entity.PreDefNotification, error) {
	var profiles []entity.PreDefNotification
