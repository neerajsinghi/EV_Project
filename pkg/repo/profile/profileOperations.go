...
func NewProfileRepository(collectionName string) ProfileRepository {
	return &repo{
...
func (r *repo) FindOne(filter, projection bson.M) (entity.ProfileDB, error) {
...
func (r *repo) Find(filter, projection bson.M) ([]entity.ProfileDB, error) {
...
// Aggregate is used to get the count of profiles
func (r *repo) Aggregate(pipeline bson.A) ([]entity.ProfileOut, error) {
	...
	defer cursor.Close(context.Background())
	var result []entity.ProfileOut
	for cursor.Next(context.TODO()) {
		var profile entity.ProfileOut
		if err = cursor.Decode(&profile); err != nil {
			trestCommon.ECLog3(
				"Find profiles",
				err,
