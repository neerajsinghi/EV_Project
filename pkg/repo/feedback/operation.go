...
func (r *repo) Find(filter, projection bson.M) ([]entity.Feedback, error) {
	var profiles []entity.Feedback
...
func (r *repo) Aggregate(pipeline bson.A) ([]entity.FeedbackOut, error) {
	var profiles []entity.FeedbackOut
	...
	defer cursor.Close(context.Background())
	for cursor.Next(context.TODO()) {
		var profile entity.FeedbackOut
		if err = cursor.Decode(&profile); err != nil {
			trestCommon.ECLog3(
				"Find profiles",
				err,
