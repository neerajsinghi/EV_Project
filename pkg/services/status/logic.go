...
func Logic(duration int) (interface{}, error) {
	pipeline := bson.A{
		// Stage 1: Calculate user statistics
		bson.D{{Key: "$facet", Value: bson.D{
			{Key: "numberOfUsers", Value: bson.A{bson.D{{Key: "$count", Value: "count"}}}},
			{Key: "idVerified", Value: bson.A{
				bson.D{{Key: "$match", Value: bson.D{{Key: "dl_verified", Value: true}}}},
				bson.D{{Key: "$count", Value: "count"}},
			}},
			{Key: "dlVerified", Value: bson.A{
				bson.D{{Key: "$match", Value: bson.D{{Key: "id_verified", Value: true}}}},
				bson.D{{Key: "$count", Value: "count"}},
			}},
		...
		// Stage 2: Calculate station statistics
		bson.D{{Key: "$lookup", Value: bson.D{
			{Key: "from", Value: "station"},
			{Key: "pipeline", Value: bson.A{
				bson.D{{Key: "$facet", Value: bson.D{
					{Key: "totalStations", Value: bson.A{bson.D{{Key: "$count", Value: "count"}}}},
					{Key: "publicStations", Value: bson.A{
						bson.D{{Key: "$match", Value: bson.D{{Key: "public", Value: true}}}},
						bson.D{{Key: "$count", Value: "count"}},
					}},
					...
					{Key: "carbonEmissionSaved", Value: bson.A{
						bson.D{{Key: "$group", Value: bson.D{
							{Key: "_id", Value: nil},
							{Key: "total", Value: bson.D{{Key: "$sum", Value: "$carbonSaved"}}},
						}}},
					}},
				}}},
			}},
		...
		// Stage 3: Calculate charger statistics
		bson.D{{Key: "$lookup", Value: bson.D{
		...
		// Stage 4: Calculate booking statistics
		bson.D{{Key: "$lookup", Value: bson.D{
			{Key: "from", Value: "booking"},
			{Key: "pipeline", Value: bson.A{
				bson.D{{Key: "$facet", Value: bson.D{
					{Key: "totalRides", Value: bson.A{bson.D{{Key: "$count", Value: "count"}}}},
		...
		// Stage 5: Calculate bike statistics
		bson.D{{Key: "$lookup", Value: bson.D{
		...
		// Stage 6: Calculate wallet statistics
		bson.D{{Key: "$lookup", Value: bson.D{
		...
		// Stage 7: Project the final result
		bson.D{{Key: "$project", Value: bson.D{
