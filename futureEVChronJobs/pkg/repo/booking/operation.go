...
func (r *repo) FindOne(filter, projection bson.M) (entity.BookingDB, error) {
	var profile entity.BookingDB
...
func (r *repo) Find(filter, projection bson.M) ([]entity.BookingDB, error) {
	var profiles []entity.BookingDB
...
func (r *repo) Count(filter bson.M) (int64, error) {
	bookingList, err := r.Find(filter, bson.M{})
	if err != nil {
		return 0, err
	}
	return int64(len(bookingList)), nil
}

func (r *repo) Aggregate(pipeline bson.A) ([]entity.BookingOut, error) {
	...
	defer cursor.Close(context.Background())
	var profiles []entity.BookingOut
	for cursor.Next(context.TODO()) {
		var profile entity.BookingOut
		if err := cursor.Decode(&profile); err != nil {
			trestCommon.ECLog3(
				"aggregate profiles",
				err,
